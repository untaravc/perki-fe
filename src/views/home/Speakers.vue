    ...
    <div class="max-w-screen-lg m-auto pt-24 pb-12 px-2" id="speaker">
        ...
        <div class="grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-8">
            <div class="group" v-for="data in data_content">
                ...
                <div>
                    <h4 class="font-bold text-base">{{ data.name }}</h4>
        ...
        <!-- <router-link to="/speakers">
            <div
                class="bg-gradient-to-r from-purple-500 to-cyan-500 p-1 text-center w-full rounded mt-4 text-violet-200 cursor-pointer">
                View all
            </div>
        </router-link> -->
    </div>
...
export default {
    ...
    methods: {
        loadSpeaker() {
            this.apiGet('pub/speakers', { ref: 'jfu25', limit: 8 })
                .then((data) => {
