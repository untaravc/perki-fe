<template>
    <div class="max-w-screen-lg m-auto pt-24 pb-12 px-2" id="speaker">
        <div class="text-4xl font-bold mb-1 text-center text-blue-900">
            SPEAKERS
        </div>
        <div class="text-small  mb-5 text-center text-blue-600">
            The speakers are experts in Cardiology and Vascular Medicine as well as related fields local and national
            level
        </div>
        <div class="grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-8">
            <div class="group" v-for="data in data_content">
                <div style="padding-bottom: 100%" :style="'background-image: url(' + data.image + ')'"
                    class="w-full bg-purple-300 bg-cover mb-2 rounded-md bg-center bg-no-repeat">
                </div>
                <div>
                    <h4 class="font-bold text-base">{{ data.name }}</h4>
                    <div class="text-xs text-neutral-material-700">{{ data.desc }}</div>
                </div>
            </div>
        </div>
        <!-- <router-link to="/speakers">
            <div
                class="bg-gradient-to-r from-purple-500 to-cyan-500 p-1 text-center w-full rounded mt-4 text-red-200 cursor-pointer">
                View all
            </div>
        </router-link> -->
    </div>
</template>

<script>
export default {
    data() {
        return {
            data_content: []
        }
    },
    methods: {
        loadSpeaker() {
            this.apiGet('pub/speakers', { ref: 'jcu25', limit: 8 })
                .then((data) => {
                    this.data_content = data.result
                })
        }
    },
    created() {
        this.loadSpeaker()
    }
}
</script>