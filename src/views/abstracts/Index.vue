<template>
    <div class="max-w-screen-lg m-auto pt-12 pb-12 px-2 mt-20" style="min-height: calc(100vh - 84px - 128px)">
        ...
        <div class="justify-center mb-4 md:flex hidden">
            <div class="bg-red-200 rounded-full flex px-4 py-2 justify-evenly">
                <div @click="loadPoster()" :class="filter.category === '' ? 'bg-red-600 text-white' : 'bg-red-200'"
                    class="  px-4 py-1 rounded-full mx-0.5 cursor-pointer hover:bg-red-300">All
                </div>
                <div @click="loadPoster('case_report')"
                    :class="filter.category === 'case_report' ? 'bg-red-600 text-white' : 'bg-red-200'"
                    class=" px-4 py-1 rounded-full mx-0.5 cursor-pointer hover:bg-red-300">Case
                    Report</div>
                <div @click="loadPoster('research')"
                    :class="filter.category === 'research' ? 'bg-red-600 text-white' : 'bg-red-200'"
                    class=" px-4 py-1 rounded-full mx-0.5 cursor-pointer hover:bg-red-300">
                    Research</div>
                <div @click="loadPoster('meta_analysis')"
                    :class="filter.category === 'meta_analysis' ? 'bg-red-600 text-white' : 'bg-red-200'"
                    class=" px-4 py-1 rounded-full mx-0.5 cursor-pointer hover:bg-red-300">Meta
                    Analisys</div>
                <div @click="loadPoster('systematic_review')"
                    :class="filter.category === 'systematic_review' ? 'bg-red-600 text-white' : 'bg-red-200'"
                    class=" px-4 py-1 rounded-full mx-0.5 cursor-pointer hover:bg-red-300">
                ...
                <router-link :to="'/posters/' + poster.id">
                    <div class="p-2 cursor-pointer">
                        ...
                        <div class="text-xs">
                            ...
                            <span class="italic text-xs text-red-500">
        ...
        <div class="flex justify-center mt-4">
            <div v-if="data_content.prev_page_url" @click="changePage('prev')"
                class="cursor-pointer p-2 pb-1 rounded m-1 bg-red-300">
